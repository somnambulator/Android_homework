...
public class ClockView extends View {
    ...
    private void drawTimeNeedles(Canvas canvas) {
         ...
         */
        //canvas.save();
        float hourDegree = hour*30.0f-90.0f;
        float hour_endX = (float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(hourDegree)));
        float hour_endY = (float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(hourDegree)));
        needlePaint.setStrokeWidth(HOUR_NEEDLE_WIDTH);
        canvas.drawLine(centerX,centerY,hour_endX,hour_endY,needlePaint);
        float minDegree = minute*6.0f-90.0f;
        float min_endX = (float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(minDegree)));
        float min_endY = (float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(minDegree)));
        needlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
        canvas.drawLine(centerX,centerY,min_endX,min_endY,needlePaint);
        float secDegree = second*6.0f-90.0f;
        float sec_endX = (float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(secDegree)));
        float sec_endY = (float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(secDegree)));
        needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
        //System.out.println(secDegree+" "+second);
